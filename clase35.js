...
async function obtenerPersonajes() {
    ...
    try {
        var personajes = await Promise.all(promesas) //llamado asincrono
...
// Async-await
// es parecido a escribir codigo de arriba hacia abajo, no tanto encadenando metodos ni callbackhell
// incluye palabras clave del lenguaje, vamos a seguir obteniendo personajes pero con Async-await

// creamos un funcion y dentro metemos el array de promesas y sus metodos y llamamos a la funcion esta igual el codigo solo que dentro de una funcion

// LO QUE VAMOS A HACER ES QUE CON EL ARRAY DE PROMESAS QUE TENEMOS
// que los personajes los vamos a querer poner en una variable y se ejecutara asincronamente
// cuando todas las promesas se resuelvan ese resultado guardalo en personajes | var personajes = Promise.all(promises)
// con await lo que hacemos es detener la ejecucion de nuestro condigo en el await 
// hasta que todas las promesas sean resueltas y recien alli ese valor se guarda en personajes
// y para poder usar await la funcion necesitamos marcarla como asincrona | async function obtenerPersonajes() {}

// PARA PODER UTILIZAR PERSONAJES 
// tenemos que meterlo entre un try y un catch
// dentro de try el await el llamado asincrono con el console.log(personajes) 
// Y el catch el error con el parametro que le mandemos de la promesa
// cuando se resuelvan todas las promesas vamos a tener el console.log(personajes)
